...
async def parse_assessment(file: UploadFile = File(...)):
    """
    Parses a WAV audio file, runs transcription and extraction pipeline, and returns FirstAssessment JSON.
    """
    ...
    try:
        ...
        # 6. Return parsed FirstAssessment
        ...
        return FirstAssessment.model_validate(assessment_dict)
...
@router.post(
    ...
    summary="Save parsed clinical assessment to MongoDB"
)
def save_assessment(assessment: FirstAssessment):
    """
    Persists a validated FirstAssessment payload into MongoDB Atlas and returns the stored record with its database ID.
    """
    try:
        inserted_id = create_assessment(assessment.model_dump())
    except ConnectionError:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Database service is unavailable. MongoDB Atlas is unreachable."
        )
    except Exception as e:
