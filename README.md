...
The same pipeline driven from `/docs` rather than the interface — upload the
WAV, execute, and read the response.

![Swagger response](docs/screenshots/10-swagger-response.png)

### The exported report

[`docs/sample-report.pdf`](docs/sample-report.pdf) — generated by the app's
*Download PDF* button, so it carries the print stylesheet rather than a browser
print of the page: the record without interface chrome, then a page break to an
appendix naming every flagged field and why it is blank, a signature line, and
a footer stating the document needs clinician review before it forms part of a
medical record.
