...
Audio → Audio validation → Whisper → Clinical transcript
...
Example:
transcript = whisper_service.transcribe("data/clinical_assessment.wav")
...
Transcript → Prompt → LLM → Structured extraction
...
Transcript → Clinical extraction → Subjective extraction → Objective extraction → Goals extraction → Plan extraction → Structured assessment
...
The main model is FirstAssessment.
...
Instead:
LLM output → Pydantic validation → FirstAssessment → Grounding → Confidence → Final JSON
...
Extracted field → Search for transcript evidence → Supported? (YES -> Accept / NO -> Flag or reject)
...
Audio → Audio validation → Whisper transcription → Clinical transcript → LangGraph extraction → Pydantic FirstAssessment → Grounding verification → Confidence scoring → Final structured JSON → MongoDB persistence
...
The exact request and response schemas are defined in app/api/schemas.py.
...
FastAPI → Service → Repository → MongoDB
...
With the sample audio: data/clinical_assessment.wav
...
The exact structure must follow the canonical FirstAssessment Pydantic contract implemented in app/schemas/assessment.py. Missing information should remain missing rather than being guessed.
...
Examples include:
* Invalid audio / missing audio file / unsupported audio format
* Whisper failure
* LLM unavailable / invalid LLM output
* Pydantic validation failure
* Grounding failure
* MongoDB unavailable
* Invalid API request / assessment not found

Errors are converted into clear application-level responses rather than exposing raw internal exceptions.
...
The system is tested across multiple levels:
* Unit tests: Individual components (audio_io, grounding, confidence, schemas, repository).
* Integration tests: Multiple components interacting (LLM -> extraction -> schema, repository -> MongoDB).
* End-to-end tests: Complete pipeline execution from audio input to persistent storage.

External services are mocked where appropriate so the test suite remains deterministic.
...
* api/: HTTP and REST concerns
* schemas/: Data contracts
* transcription/: Speech recognition
* extraction/: LLM and clinical extraction
...
Clinical information can be sensitive. The application follows these principles:
* Do not hard-code secrets or commit .env.
* Keep provider credentials outside source code.
* Avoid logging sensitive clinical content unnecessarily.
* Validate incoming files and structured outputs.
* Prevent unsupported LLM-generated information from being accepted automatically.

For local deployments, Whisper and Ollama can run locally so audio and transcript data do not leave the host machine.
...
* Testable & Replaceable: Modular design allows swapping infrastructure without rewriting domain logic.
...
Start MongoDB and Ollama (if using local inference), then launch the API:
uvicorn app.main:app --reload
...
A future production deployment can separate services while preserving core domain models:

Frontend ──► API Gateway ──► FastAPI ──► Pipeline Service
                                             ├── Transcription Service
                                             ├── Extraction Service
                                             ├── Grounding Service
                                             └── Confidence Service ──► MongoDB
...
* Additional audio formats and STT providers
* Hosted LLM API providers
* AuthN/AuthZ & Role-based access control
* Background queue processing (Celery/Redis)
* Assessment versioning & audit logs
...
Audio ──► Transcript ──► Evidence-based extraction ──► FirstAssessment ──► Grounding ──► Confidence ──► JSON DB

At no point should unsupported clinical information be invented. If evidence is lacking, fields are represented as missing or uncertain according to the FirstAssessment contract.
...
The result is a modular, testable, configurable, and evidence-grounded clinical assessment pipeline.
